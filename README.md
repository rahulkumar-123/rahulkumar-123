<h1 align="center">
    <img src="https://readme-typing-svg.herokuapp.com/?font=Inter&size=48&center=true&vCenter=true&width=500&height=70&color=4493F8&duration=4000&lines=Hi+There!+👋;+I'm+Rahul+Kumar!;" />
</h1>
<h1 align="center">Rahul Kumar</h1>

<p align="center">
Dual Degree (B.Tech + M.Tech) - Electronics & Communication Engineering  
National Institute of Technology, Patna
</p>

<p align="center">
I enjoy building systems close to the metal - from RTL and digital design to high-performance C++.
</p>

### What I'm working on

- Studying **computer architecture and system design**
- Writing **high-performance C++**
- Exploring **RTL design and hardware verification**
- Solving problems on **LeetCode and Codeforces**

---

### A few things I've built

**High-Performance In-Memory Cache Engine**

C++ implementation of an LRU cache designed for speed and concurrency.  
O(1) get/put operations with multithreaded synchronization and performance monitoring.

**AXI4-Stream Synchronous FIFO**

RTL design written in Verilog with AXI4-Stream integration.  
Includes a C++ golden reference model and automated verification setup.

**Alumni Portal**

Full-stack MERN application with authentication, real-time chat, notifications, and a community feed.

---

### Tools I work with

**Languages**

C++  
C  
Verilog / SystemVerilog  
JavaScript

**Concepts**

Data Structures & Algorithms  
Concurrency  
Computer Architecture  
Digital Design  
Operating Systems
Full-Stack Development

**Tools**

Linux  
Git  
ModelSim  
GTKWave  
Vivado / Quartus

---

### Competitive Programming

- 600+ problems solved  
- **LeetCode Knight (1900+)**
- **Codeforces Specialist**

---

### GitHub Stats

<p align="center">
<img src="https://github-readme-stats.vercel.app/api?username=rahulkumar-123&show_icons=true&theme=tokyonight"/>
</p>

<p align="center">
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=rahulkumar-123&layout=compact&theme=tokyonight"/>
</p>

---

### Connect

<p align="center">

<a href="mailto:rahulpd180@gmail.com">
<img src="https://img.shields.io/badge/email-contact-red?style=for-the-badge&logo=gmail"/>
</a>

<a href="https://linkedin.com/in/rahulkmr-nitp">
<img src="https://img.shields.io/badge/linkedin-profile-blue?style=for-the-badge&logo=linkedin"/>
</a>

</p>

---

<p align="center">
<img src="https://komarev.com/ghpvc/?username=rahulkumar-123&label=Profile%20Views&color=blue"/>
</p>
